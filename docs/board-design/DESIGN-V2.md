...
motion triggers) stays as written in the board design brief, a separate planning document not included in this repository (called "the brief" below, cited as `brief §N`).
This file replaces the brief only where the table "Deviations" (section 12) says so.

Mockups in this folder (all show one illustrative demo run: 16 tasks, 14 done, T20 needs your answer, T21 waiting on it, 1.2M tokens):
...
```
ALE 2026-09-22-auth-refresh-demo  ■ Stalled: needs you
Nothing is running. 1 needs you; 1 is waiting behind them.
✓✓✓✓✓✓✓✓✓✓✓✓✓✓?·  14 of 16 done   1.2M tok, run total   cost not reported
Needs you 1
? T20      Needs your answer Remove legacy session store (needs review)
...
Done 14
✓ T14      Done              Add token refre… Done 1h 58m ago
✓ T13      Done              Write auth inte… Done 2h 20m ago
✓ T12      Done              Document the se… Done 2h 51m ago
  +11 more done
...
Done 14
✓ T14      Done              Add token refresh endpoint       Done 1h 58m ago                                   8.1k tok
✓ T13      Done              Write auth integration tests     Done 2h 20m ago                                   9.8k tok
✓ T12      Done              Document the session API         Done 2h 51m ago                                    11k tok
  +11 more done
board: http://127.0.0.1:43127/<token>/  (copy into a browser)
```
...
every task, token and time value here is illustrative demo data.
...
| Fault in the previous board's screenshots | Fix |
|---|---|
