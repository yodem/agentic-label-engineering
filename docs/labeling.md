...
| `lane` | Noul `large_change`; facts `unattended` (run property, default false), `role`, `independent_tasks` | large change -> `pane`; unattended run -> `pane`; role test or review -> `workflow`; two or more independent tasks -> `workflow`; else `inline` (the `flow lane` table) | `inline`, `workflow`, `pane` |
