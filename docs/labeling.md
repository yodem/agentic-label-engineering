...
| `lane` | Nouls `large_change`, `needs_person`; facts `role`, `independent_tasks` | large change -> `pane`; no named person step (unattended) -> `pane`; role test or review -> `workflow`; two or more independent tasks -> `workflow`; else `inline` (the `flow lane` table) | `inline`, `workflow`, `pane` |
