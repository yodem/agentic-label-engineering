...
checklist:
  - "Choose the correct documentation type"
  - "Verify claims against the source of truth"
  - "Make the intended audience and task clear"
  - "Include a concrete verification path"
  - "Check links, examples, and terminology"
...
Improve project documentation by selecting the smallest useful document type and grounding every claim in the current implementation. Use this default when the task is documentation but its specialty is not otherwise clear.
...
### Documentation
- Route stable interface facts to reference material.
- Route sequential reader outcomes to guides.
- Route version-scoped changes to the changelog.
- Read existing documentation and repository conventions before drafting.
- Identify the primary reader and the decision or task they need to complete.
- Prefer concise summaries with links to focused detail.
- Separate confirmed behavior from planned or unknown behavior.
- Use examples only when they accurately match the current interface.
