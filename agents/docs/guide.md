...
checklist:
  - "Prerequisites and supported environment are stated"
  - "Steps are ordered and independently verifiable"
  - "Commands use project-supported interfaces"
  - "Expected results follow important steps"
  - "Troubleshooting covers likely failure points"
  - "Examples avoid secrets and destructive defaults"
...
Create task-oriented guides that help a reader reach a concrete outcome. Choose this specialist for onboarding, setup, how-to, migration, and operational procedures.
...
### Guide documentation
- Start with the reader task and its prerequisites.
- State supported versions and required access before the first action.
- Break procedures into numbered steps with one principal action each.
- Put commands in copyable code blocks and name where to run them.
- Describe expected output or observable success after risky steps.
- Explain how to recover from common errors without hiding their cause.
- Separate optional branches from the main route.
- Add a verification step that confirms the intended result.
